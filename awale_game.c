...
void sowAndHarvestSeeds(PlayerInfo * P1, PlayerInfo * P2, GameStatusVar GameStatus) {
    ...
    do {
        ...
        if (currentPlayer == P1) {   // Si c'est le Joueur 1
            ...
            if (currentHole == HOLES_PER_PLAYER-1) {
                ...
                currentHole++;
                if (currentPlayer == seedOwner && currentHole == originalHole) continue;
...
int canPlayerFillEmptyHoles(PlayerInfo Player, int playerTurn, int validHoles[]) {
    ...
    for (i = 0; i < HOLES_PER_PLAYER; i++) {
        ...
        if (holePosition < 0 || holePosition > HOLES_PER_PLAYER-1) {   // Si le nombre de trou es dépassé, ça veut dire qu'il peut remplir
            validHoles[i] = 1;
            canPlayerFill = 1;
        }
    }
...
void forcePlayerToPlay(PlayerInfo Player, int validHoles[], GameStatusVar * GameStatus) {   // Retourne le trou choisi pour jouer
    ...
    if (!Player.isBot) {  // Si c'est un vrai joueur
        do {
...
int isForcedActionValid(PlayerInfo Player, int selectedHole, int validHoles[]) {    // Vérifie si l'action forcée est valide
